...
const getFarmerDashboard = async (req, res) => {
  try {
    ...
    const [pendingSelections, negotiationReadySelections, rejectedSelections] =
      await Promise.all([
    ...
    // Recent selections
    const recentSelections = await MatchSelection.find({
      farmerId,
    })
      .populate("millerId", "name millName district location")
      .populate("demandId", "paddyType quantityNeeded offeredPrice status")
      .populate(
        "harvestId",
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        marketAnalytics: {
          totalQuantity: Number(analytics.totalQuantity || 0),

          averageExpectedPrice: Number(
            (analytics.averageExpectedPrice || 0).toFixed(2),
          ),

          averageAiPredictedPrice: Number(
            (analytics.averageAiPredictedPrice || 0).toFixed(2),
          ),

          averageHarvestScore: Number(
            (analytics.averageHarvestScore || 0).toFixed(2),
          ),
        },

        latestAiRecommendation: latestAiHarvest,
...
const getMillerDashboard = async (req, res) => {
  try {
    ...
    // Demand statistics
    const [totalDemands, openDemands, closedDemands] = await Promise.all([
    ...
    // Demand analytics
    const demandAnalytics = await MillerDemand.aggregate([
      {
        $match: {
          millerId: new mongoose.Types.ObjectId(millerId),
        },
      },
      {
        $group: {
    ...
    // Recent demands
    const recentDemands = await MillerDemand.find({
      millerId,
    })
      .sort({ createdAt: -1 })
      .limit(5)
    ...
    const recentSelections = await MatchSelection.find({
      ...
      .populate("farmerId", "farmerName district location")
      .populate(
        "harvestId",
    ...
    const recommendedFarmerMatches = await MatchSelection.find({
      millerId,
      status: "pending",
    })
      .populate("farmerId", "farmerName district location")
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        marketAnalytics: {
          totalQuantityNeeded: Number(analytics.totalQuantityNeeded || 0),

          averageOfferedPrice: Number(
            (analytics.averageOfferedPrice || 0).toFixed(2),
          ),
        },
