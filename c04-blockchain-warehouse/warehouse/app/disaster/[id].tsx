...
import { disasterService, Disaster, RankedCandidate } from "../../services/disaster.service";
  ...
  const handleRedistribute = (candidate: RankedCandidate) => {
    Alert.prompt(
      ...
      async (qty) => {
        ...
        try {
          await disasterService.redistribute(id, candidate.warehouseId, quantity);
          Alert.alert("Success", "Redistribution order issued and anchored on blockchain");
          load();
        } catch (err: any) {
          Alert.alert("Error", err?.response?.data?.message || "Failed to issue order");
        }
      },
      "plain-text"
    );
    ...
    <View style={styles.screen}>
      ...
      <View style={styles.header}>
        ...
        <View style={styles.headerRow}>
          <Text style={styles.headerIcon}>{DISASTER_ICONS[disaster.disasterType] ?? "⚠️"}</Text>
          <View>
            <Text style={styles.headerTitle}>{disaster.disasterType.replace("_", " ")}</Text>
            <Text style={styles.headerSubtitle}>{disaster.affectedWarehouse.name}</Text>
          </View>
        </View>
      </View>
      ...
      >
        <View style={styles.content}>
          ...
          <View style={styles.card}>
            ...
            <View style={styles.statsRow}>
              <View style={styles.statItem}>
                ...
                <Text style={styles.statValue}>{disaster.estimatedLossTons ?? "—"} tons</Text>
              </View>
              <View style={styles.statItem}>
                <Text style={styles.statLabel}>Reported by</Text>
                <Text style={styles.statValue}>{disaster.reportedBy.fullName}</Text>
              </View>
            </View>
            {disaster.blockchainTxId && (
              <View style={styles.chainRow}>
                <Ionicons name="lock-closed" size={12} color={COLORS.info} />
                <Text style={styles.chainText}>Anchored on Hyperledger Fabric</Text>
              </View>
            )}
          </View>
            ...
            <View style={styles.card}>
              ...
              {disaster.zkpProofs.map((proof: any) => (
                <View key={proof.id} style={styles.proofRow}>
                  ...
                  />
                  <Text style={styles.proofId}>{proof.warehouseId.slice(0, 8)}...</Text>
                  <Text style={[
                    styles.proofStatus,
                    { color: proof.verificationResult ? COLORS.success : COLORS.danger }
                  ]}>
          ...
          {/* Ranked Candidates */}
          {disaster.rankedCandidates && disaster.rankedCandidates.length > 0 && (
                ...
                <View key={c.warehouseId} style={styles.candidateCard}>
                  <View style={styles.candidateHeader}>
                    <View style={styles.candidateHeaderLeft}>
                      ...
                      <View style={styles.candidateInfo}>
                        ...
                        <Text style={styles.candidateSubtitle}>{c.code} · {c.district}</Text>
                      </View>
                    </View>
                    <View style={styles.scoreBox}>
                      <Text style={styles.scoreValue}>{c.compositeScore.toFixed(3)}</Text>
                  ...
                  <View style={styles.candidateStatsRow}>
                    <View style={styles.candidateStatItem}>
                      ...
                      <Text style={styles.candidateStatValue}>{c.distanceKm} km</Text>
                    </View>
                    <View style={styles.candidateStatItem}>
                      <Text style={styles.candidateStatLabel}>Available</Text>
                      <Text style={[styles.candidateStatValue, { color: COLORS.info }]}>{c.availableTons} tons</Text>
                    </View>
                    <View style={styles.candidateStatItem}>
                    ...
                    <View style={styles.candidateStatItem}>
                      ...
                      <View style={styles.zkpRow}>
                        <Ionicons
                          name={c.zkpVerified ? "shield-checkmark" : "shield-outline"}
                          size={12}
                          color={c.zkpVerified ? COLORS.success : COLORS.textFaint}
                        />
                        <Text style={[
                          styles.zkpText,
                          { color: c.zkpVerified ? COLORS.success : COLORS.textFaint }
                  ...
                  {c.canFulfil && disaster.status !== "RESOLVED" && (
                    ...
                    >
                      <Text style={styles.redistributeButtonText}>Issue Redistribution Order</Text>
                    </TouchableOpacity>
                  )}
                  {!c.canFulfil && (
                    <View style={styles.insufficientBadge}>
                      <Text style={styles.insufficientText}>Insufficient capacity</Text>
                    </View>
                  )}
                </View>
              ))}
...
const styles = StyleSheet.create({
  ...
  card: { backgroundColor: COLORS.bgCard, borderRadius: 12, padding: 16, marginBottom: 16, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 4, shadowOffset: { width: 0, height: 1 }, elevation: 1 },
  cardTitle: { color: COLORS.textSecondary, fontWeight: "bold" },
  cardTitleBlock: { color: COLORS.textSecondary, fontWeight: "bold", marginBottom: 12 },
  ...
  chainRow: { flexDirection: "row", alignItems: "center", marginTop: 12, paddingTop: 12, borderTopWidth: 1, borderTopColor: COLORS.borderLight },
  chainText: { color: COLORS.info, fontSize: 12, marginLeft: 4 },

  proofRow: { flexDirection: "row", alignItems: "center", paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: COLORS.borderLight },
  ...
  candidateCard: { backgroundColor: COLORS.bgCard, borderRadius: 12, padding: 16, marginBottom: 12, shadowColor: "#000", shadowOpacity: 0.05, shadowRadius: 4, shadowOffset: { width: 0, height: 1 }, elevation: 1 },
  candidateHeader: { flexDirection: "row", alignItems: "flex-start", justifyContent: "space-between", marginBottom: 8 },
  candidateHeaderLeft: { flexDirection: "row", alignItems: "center", flex: 1 },
  rankBadge: { width: 28, height: 28, borderRadius: 14, backgroundColor: COLORS.successBg, alignItems: "center", justifyContent: "center", marginRight: 8 },
  rankBadgeText: { color: COLORS.primaryDark, fontSize: 12, fontWeight: "bold" },
  ...
  candidateStatValue: { color: COLORS.textSecondary, fontSize: 12, fontWeight: "500" },
  zkpRow: { flexDirection: "row", alignItems: "center" },
  zkpText: { fontSize: 12, marginLeft: 4 },

  redistributeButton: { backgroundColor: COLORS.primary, borderRadius: 8, paddingVertical: 8, alignItems: "center" },
  redistributeButtonText: { color: COLORS.white, fontSize: 12, fontWeight: "bold" },
  insufficientBadge: { backgroundColor: COLORS.borderLight, borderRadius: 8, paddingVertical: 8, alignItems: "center" },
  insufficientText: { color: COLORS.textFaint, fontSize: 12 },

  orderRow: { paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: COLORS.borderLight },
  ...
  resolveButton: { backgroundColor: COLORS.textSecondary, borderRadius: 12, paddingVertical: 16, alignItems: "center", marginBottom: 16 },
